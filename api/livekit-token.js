...
export const liveKitGrant = (role, room) => ({
  room, roomJoin:true, canSubscribe:role === "viewer", canPublish:role === "owner",
  canPublishData:false,
  ...(role === "owner" ? {canPublishSources:[TrackSource.CAMERA,TrackSource.MICROPHONE]} : {}),
});
...
export default async function handler(req, res) {
  ...
  try {
    ...
    token.addGrant(liveKitGrant(role,room));
    ...
    return json(res,500,{error:"Camera relay token generation failed"});
  }
}
