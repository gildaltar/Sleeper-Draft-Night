...
import { avatarUrl, draftSlotForMember, parsePanelProfile, playerImage, rosterNeeds } from "../lib/draft";
import HelmetIdentity from "./HelmetIdentity";

export default function Teams({ league, draft, members, picks, profiles }) {
        ...
        {members.map((member) => {
          ...
          return (
            <article className={`team-hq-card style-${panel.style}`} style={{ "--team":profile?.accent || `hsl(${member.rosterId * 54} 76% 58%)`, "--team-2":profile?.accent_2 || "#b7ff3c" }} key={member.userId}>
              <header><HelmetIdentity profile={profile} member={member} /><div><span>{profile?.badge || `TEAM ${member.rosterId}`}</span><h2>{profile?.team_name || member.teamName}</h2><p>{profile?.motto || "Ready for draft night"}</p></div><strong>{draftSlot}</strong></header>
              <div className="team-owner"><div className="team-avatar"><span>{member.displayName.slice(0,1).toUpperCase()}</span>{member.avatar && <img src={avatarUrl(member.avatar)} alt="" onError={(event)=>event.currentTarget.remove()} />}</div><div><small><UserRound />OWNER</small><b>{member.displayName}</b><span>Draft slot {draftSlot}</span></div><div><small><ListChecks />ROSTERED</small><b>{drafted.length}</b><span>of {league.rosterPositions.length} spots</span></div></div>
