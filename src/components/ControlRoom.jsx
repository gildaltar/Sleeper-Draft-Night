    ...
    <main className="control-room content-view">
      ...
      <div className="control-grid">
        ...
        <article className="control-card">
          ...
          <p>One Sound On click starts the Spotify auto queue. Music pauses for draft start, pick-in, reveal, announcement, and draft-end cues, then resumes automatically.</p>
        </article>
        <article className="control-card show-control">
          ...
          <div className="overlay-form"><label>Type<select value={overlay.type} onChange={(event)=>setOverlay({...overlay,type:event.target.value})}><option value="announcement">Announcement</option><option value="trade">Trade alert</option><option value="round">Round break</option><option value="celebration">Celebration</option><option value="alert">Urgent alert</option></select></label><label>Kicker<input maxLength="32" value={overlay.kicker} onChange={(event)=>setOverlay({...overlay,kicker:event.target.value})}/></label><label className="wide">Headline<input maxLength="84" value={overlay.title} onChange={(event)=>setOverlay({...overlay,title:event.target.value})}/></label><label className="wide">Details<textarea maxLength="180" value={overlay.detail} onChange={(event)=>setOverlay({...overlay,detail:event.target.value})}/></label><label>Display time<input type="number" min="2" max="30" value={overlay.duration} onChange={(event)=>setOverlay({...overlay,duration:event.target.value})}/></label><label>Sound<select value={overlay.sound} onChange={(event)=>setOverlay({...overlay,sound:event.target.value})}><option value="announcement">Announcement chime</option><option value="alert">Alert stinger</option><option value="fanfare">Fanfare</option><option value="none">No sound</option></select></label></div>
