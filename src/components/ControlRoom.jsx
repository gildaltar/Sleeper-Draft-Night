  ...
  return (
    <main className="control-room content-view control-console">
      ...
      <div className="control-grid control-tab-panel">
        ...
        {activeTab === "audio" && <article className="control-card control-card-wide control-audio-card"><h2><Volume2 />Event sound library & mix</h2><p>Upload the exact file for each draft event, test it, and set its independent volume. Missing files use a safe synthesized fallback.</p><DraftAudio picks={live?.picks || []} draftStatus={live?.draft?.status || bootstrap.draft.status} announcement={control.state.announcement} panel /></article>}
